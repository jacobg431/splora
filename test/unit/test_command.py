...
class TestContract:
    ...
    def test_the_contract_itself_cannot_be_instantiated(self):
        with pytest.raises(TypeError):
            Command()

    def test_a_complete_command_can_be_instantiated(self):
        assert isinstance(_Complete(), Command)

    def test_a_command_missing_a_member_cannot_be_instantiated(self):
        with pytest.raises(TypeError):
            _WithoutAbandon()

    def test_the_refusal_names_the_missing_member(self):
        with pytest.raises(TypeError) as exc:
            _WithoutAbandon()
        assert "abandon" in str(exc.value)

    def test_a_complete_command_reports_its_outcome(self):
