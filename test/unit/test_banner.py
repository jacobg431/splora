...
class TestBannerShape:
    ...
    def test_renders_the_art_and_the_footer(self):
        assert len(_rendered().splitlines()) == _ART_ROWS + 2

    def test_the_emblem_never_reaches_into_the_wordmark(self):
        for row in _rendered().splitlines()[:_ART_ROWS]:
            assert set(row[:_EMBLEM_COLUMNS]) <= set(" #%+.")

    def test_a_gutter_separates_the_two(self):
        for row in _rendered().splitlines()[:_ART_ROWS]:
            assert row[_GUTTER_COLUMNS] == "  "

    def test_every_art_row_carries_a_wordmark_stroke(self):
        for row in _rendered().splitlines()[:_ART_ROWS]:
            assert any(stroke in row for stroke in "_|/\\")

    def test_output_is_entirely_ascii(self):
        assert _rendered().isascii()

    def test_colored_output_is_entirely_ascii(self):
...
class TestBannerColor:
    ...
    def test_monochrome_carries_no_escape(self):
        assert _ESCAPE not in _rendered(use_color=False)

    def test_color_carries_escapes(self):
        assert _ESCAPE in _rendered(use_color=True)

    def test_stripping_the_color_yields_the_monochrome_banner(self):
        assert _ANY_ESCAPE.sub("", _rendered(use_color=True)) == _rendered(use_color=False)

    def test_each_density_step_uses_its_own_color(self):
    ...
    def test_the_ramp_is_four_distinct_colors(self):
        assert len({ACCENT_LIGHT, ACCENT, ACCENT_DEEP, ACCENT_DIM}) == 4

    def test_monochrome_keeps_every_density_step(self):
...
class TestVersionFallback:
    ...
    def test_the_fallback_is_ascii(self, monkeypatch):
        def absent(_name: str) -> str:
            raise metadata.PackageNotFoundError
    ...
    def test_reports_what_the_metadata_says_when_present(self, monkeypatch):
