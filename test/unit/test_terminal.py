...
class TestOutputConfig:
    ...
    def test_carries_both_fields(self) -> None:
...
class TestPaint:
    ...
    def test_returns_the_text_unchanged_when_color_is_off(self) -> None:
        assert paint("hello", ACCENT, use_color=False) == "hello"

    def test_writes_no_escape_when_color_is_off(self) -> None:
        assert _ESCAPE not in paint("hello", ACCENT, use_color=False)

    def test_wraps_the_text_when_color_is_on(self) -> None:
        assert paint("hello", ACCENT, use_color=True) == f"\x1b[{ACCENT}mhello\x1b[0m"

    def test_resets_the_color_after_the_text(self) -> None:
        assert paint("hello", ACCENT, use_color=True).endswith("\x1b[0m")

    def test_keeps_the_text_intact(self) -> None:
        assert "hello" in paint("hello", WARNING, use_color=True)

    def test_uses_the_color_it_is_given(self) -> None:
...
class TestFormatBytes:
    ...
    def test_exactly_one_pb(self) -> None:
        assert format_bytes(1024**5) == "1.0 PB"

    def test_large_byte_value(self) -> None:
...
class TestNoticeLine:
    ...
    def test_trimmed_output_is_the_bare_message(self) -> None:
        config = OutputConfig(trim=True, use_color=False)
        assert notice_line("Stopped.", config=config) == "Stopped."

    def test_trimmed_output_carries_no_glyph(self) -> None:
        config = OutputConfig(trim=True, use_color=True)
        assert notice_line("Stopped.", config=config) == "Stopped."

    def test_trimmed_output_carries_no_escape(self) -> None:
        config = OutputConfig(trim=True, use_color=True)
        assert _ESCAPE not in notice_line("Stopped.", config=config)

    def test_styled_output_carries_a_glyph(self) -> None:
        config = OutputConfig(trim=False, use_color=False)
        assert notice_line("Stopped.", config=config) == "! Stopped."

    def test_styled_output_without_color_carries_no_escape(self) -> None:
        config = OutputConfig(trim=False, use_color=False)
        assert _ESCAPE not in notice_line("Stopped.", config=config)

    def test_styled_output_with_color_carries_an_escape(self) -> None:
        config = OutputConfig(trim=False, use_color=True)
        assert _ESCAPE in notice_line("Stopped.", config=config)

    def test_styled_output_keeps_the_message(self) -> None:
        config = OutputConfig(trim=False, use_color=True)
        assert "Stopped." in notice_line("Stopped.", config=config)

    def test_the_message_is_never_dropped(self) -> None:
        for config in (
...
class TestVirtualTerminal:
    ...
    def test_adds_the_virtual_terminal_flag(self) -> None:
        assert _with_virtual_terminal(0) == _ENABLE_VIRTUAL_TERMINAL_PROCESSING

    def test_preserves_the_flags_already_in_force(self) -> None:
