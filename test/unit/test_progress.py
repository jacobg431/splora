...
class TestShouldRedraw:
    ...
    def test_redraws_when_it_has_never_been_drawn(self):
        assert _should_redraw(None, 0.0) is True

    def test_holds_back_before_the_interval_elapses(self):
        assert _should_redraw(0.0, _REDRAW_INTERVAL / 2) is False

    def test_redraws_once_the_interval_elapses(self):
        assert _should_redraw(0.0, _REDRAW_INTERVAL) is True

    def test_redraws_well_after_the_interval(self):
        assert _should_redraw(0.0, _REDRAW_INTERVAL * 10) is True

    def test_holds_back_when_no_time_has_passed(self):
...
class TestRenderingGate:
    ...
    def test_writes_nothing_when_the_stream_is_not_a_terminal(self):
        stream = io.StringIO()
    ...
    def test_finishing_writes_nothing_when_the_stream_is_not_a_terminal(self):
        stream = io.StringIO()
        progress = Progress(stream, use_color=False, clock=_Clock())
        progress.record(1024)
        progress.finish()
...
class TestThrottling:
    ...
    def test_draws_on_the_first_file(self):
        progress, stream = _on_terminal()
        progress.record(1)
        assert stream.getvalue().count("\r") == 1

    def test_holds_back_within_the_interval(self):
        clock = _Clock()
    ...
    def test_draws_again_once_the_interval_elapses(self):
        clock = _Clock()
        progress, stream = _on_terminal(clock)
        progress.record(1)
    ...
    def test_counts_every_file_regardless_of_drawing(self):
        clock = _Clock()
...
class TestLine:
    ...
    def test_groups_large_file_counts(self):
        progress, _ = _on_terminal()
        for _ in range(1000):
            progress.record(1)
        assert "1,000 files" in progress.line(0.0)

    def test_reports_the_cumulative_size(self):
        progress, _ = _on_terminal()
        progress.record(1024)
        progress.record(1024)
        assert "2.0 KB" in progress.line(0.0)

    def test_reports_the_elapsed_time(self):
        clock = _Clock()
        progress, _ = _on_terminal(clock)
        progress.record(1)
        assert "1.5s" in progress.line(1.5)

    def test_reports_the_throughput(self):
        clock = _Clock()
        progress, _ = _on_terminal(clock)
        progress.record(1024)
        assert "1.0 KB/s" in progress.line(1.0)

    def test_reports_no_throughput_before_any_time_passes(self):
        progress, _ = _on_terminal()
        progress.record(1024)
...
class TestFinish:
    ...
    def test_erases_what_was_drawn(self):
        clock = _Clock()
        progress, stream = _on_terminal(clock)
        progress.record(1024)
    ...
    def test_returns_to_the_row_start_when_nothing_was_drawn(self):
        progress, stream = _on_terminal()
        progress.finish()
        assert stream.getvalue() == "\r\r"

    def test_leaves_the_cursor_at_the_start_of_the_row(self):
        progress, stream = _on_terminal()
        progress.record(1024)
        progress.finish()
...
class TestColor:
    ...
    def test_color_leaves_the_counts_readable(self):
        stream = _Terminal()
        progress = Progress(stream, use_color=True, clock=_Clock())
        progress.record(1024)
