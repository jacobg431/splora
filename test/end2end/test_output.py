...
class TestDecoratedOutput:
    ...
    def test_the_banner_is_printed(self, attempt_cli, tmp_path, scratch_run) -> None:
    ...
    def test_the_next_step_is_printed(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
    ...
    def test_a_whole_scan_exits_zero(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
...
class TestTrimmedOutput:
    ...
    def test_the_banner_is_suppressed(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
    ...
    def test_the_next_step_is_suppressed(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
        name = scratch_run("trimmed-advice")
        result = attempt_cli("explore", str(scan), "--name", name, "--trim-output")
        assert b"Next:" not in result.stdout

    def test_the_result_summary_survives(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
    ...
    def test_a_report_keeps_its_summary(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
...
class TestExitCodes:
    ...
    def test_a_capped_scan_reports_partial(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
        name = scratch_run("capped")
        result = attempt_cli("explore", str(scan), "--name", name, "--max-files", "1")
        assert result.returncode == 3

    def test_a_missing_path_is_a_user_error(self, attempt_cli, tmp_path, scratch_run) -> None:
        result = attempt_cli("explore", str(tmp_path / "absent"), "--name", scratch_run("absent"))
        assert result.returncode == 1

    def test_a_flag_before_its_subcommand_is_a_usage_error(self, attempt_cli, tmp_path) -> None:
        result = attempt_cli("--trim-output", "explore", str(tmp_path))
        assert result.returncode == 2

    def test_an_unknown_report_is_a_user_error(self, attempt_cli, scratch_run) -> None:
...
class TestRedirectedOutput:
    ...
    def test_every_byte_of_a_scan_is_ascii(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
        result = attempt_cli("explore", str(scan), "--name", scratch_run("ascii"))
        assert result.stdout.isascii()
        assert result.stderr.isascii()

    def test_every_byte_of_a_report_is_ascii(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
    ...
    def test_a_partial_scan_is_ascii(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
        name = scratch_run("ascii-partial")
        result = attempt_cli("explore", str(scan), "--name", name, "--max-files", "1")
        assert result.stdout.isascii()

    def test_no_escape_sequence_reaches_a_pipe(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
        result = attempt_cli("explore", str(scan), "--name", scratch_run("nocolor"))
        assert _ESCAPE not in result.stdout

    def test_no_progress_line_reaches_a_pipe(self, attempt_cli, tmp_path, scratch_run) -> None:
        scan = _make_tree(tmp_path)
