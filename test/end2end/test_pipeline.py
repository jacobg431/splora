...
class TestExplore:
    ...
    def test_json_file_is_created(self, e2e_pipeline) -> None:
        assert e2e_pipeline["json_path"].exists()

    def test_json_is_not_partial(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
        assert data["meta"]["partial"] is False

    def test_run_name_matches(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
        assert data["meta"]["name"] == e2e_pipeline["run_name"]

    def test_root_path_matches_scan_directory(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
        assert data["meta"]["root"] == str(e2e_pipeline["scan_root"])

    def test_total_file_count_is_correct(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
        assert data["meta"]["total_files"] == 5

    def test_tree_has_one_subdirectory(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
        assert len(data["tree"]["children"]) == 1
        assert data["tree"]["children"][0]["name"] == "subdir"

    def test_extension_distribution_is_correct(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
    ...
    def test_category_distribution_is_correct(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
    ...
    def test_total_size_matches_actual_files(self, e2e_pipeline) -> None:
        data = _load_json(e2e_pipeline["json_path"])
...
class TestReport:
    ...
    def test_regenerating_report_removes_stale_files(self, e2e_pipeline, run_cli) -> None:
...
class TestBoot:
    ...
    def test_server_responds_on_the_localhost_url_boot_opens(self, e2e_pipeline) -> None:
        resp = urllib.request.urlopen(e2e_pipeline["localhost_url"], timeout=5)
        assert resp.status == 200

    def test_index_html_is_served(self, e2e_pipeline) -> None:
        body = _fetch(e2e_pipeline).read().decode()
        assert "<html" in body.lower()

    def test_stylesheet_is_served(self, e2e_pipeline) -> None:
        resp = _fetch(e2e_pipeline, "style.css")
        assert resp.status == 200

    def test_main_module_is_served(self, e2e_pipeline) -> None:
        resp = _fetch(e2e_pipeline, "main.js")
        assert resp.status == 200

    def test_data_json_is_served(self, e2e_pipeline) -> None:
        resp = _fetch(e2e_pipeline, "data.json")
        assert resp.status == 200

    def test_data_json_has_correct_run_name(self, e2e_pipeline) -> None:
        body = _fetch(e2e_pipeline, "data.json").read()
        data = json.loads(body.decode())
        assert data["meta"]["name"] == e2e_pipeline["run_name"]

    def test_data_json_file_count_is_correct(self, e2e_pipeline) -> None:
        body = _fetch(e2e_pipeline, "data.json").read()
        data = json.loads(body.decode())
        assert data["meta"]["total_files"] == 5

    def test_module_is_served(self, e2e_pipeline) -> None:
        resp = _fetch(e2e_pipeline, "core/theme.js")
        assert resp.status == 200
        assert "javascript" in resp.headers.get("Content-Type", "")

    def test_unknown_path_returns_404(self, e2e_pipeline) -> None:
        with pytest.raises(urllib.error.HTTPError) as exc:
            _fetch(e2e_pipeline, "nonexistent.html")
        assert exc.value.code == 404

    def test_parent_directory_traversal_is_blocked(self, e2e_pipeline) -> None:
